...
This plateau indicates the bottleneck is not Postgres CPU or memory.
At 100 concurrent VUs from remote k6 runners, the floor is set by
...
beyond Small provides no benefit because the bottleneck shifts to
network latency.  To push latency lower, the load generators would
need to be co-located with the database (same region, same VPC).
